 ...
 */
public class MessageFlowShapeController extends AbstractBusinessObjectShapeController {
  
  public MessageFlowShapeController(ActivitiBPMNFeatureProvider featureProvider) {
  ...
  public PictogramElement createShape(Object businessObject, ContainerShape layoutParent, int width, int height, IAddContext context) {
    ...
    MessageFlow addedMessageFlow = (MessageFlow) context.getNewObject();
    ...
    if(addConContext.getSourceAnchor() == null) {
      ...
      for (Shape shape : shapeList) {
        ...
        if (flowNode == null || flowNode.getId() == null || addedMessageFlow.getSourceRef() == null ||
            addedMessageFlow.getTargetRef() == null) continue;
        if (flowNode.getId().equals(addedMessageFlow.getSourceRef())) {
        ...
        if (flowNode.getId().equals(addedMessageFlow.getTargetRef())) {
          EList<Anchor> anchorList = ((ContainerShape) shape).getAnchors();
          for (Anchor anchor : anchorList) {
            if (anchor instanceof ChopboxAnchor) {
              targetAnchor = anchor;
    ...
    FlowElement sourceElement = model.getFlowElement(addedMessageFlow.getSourceRef());
    FlowElement targetElement = model.getFlowElement(addedMessageFlow.getTargetRef());
    ...
    String name = BpmnExtensionUtil.getMessageFlowName(addedMessageFlow, ActivitiPlugin.getDefault());
    text.setValue(name);
  ...
  protected Polygon createArrow(GraphicsAlgorithmContainer gaContainer, Diagram diagram) {
    ...
    polyline.setStyle(StyleUtil.getStyleForMessageFlow(diagram));
    return polyline;
  }
