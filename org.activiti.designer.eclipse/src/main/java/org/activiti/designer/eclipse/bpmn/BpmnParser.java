...
public class BpmnParser {
      ...
      while (xtr.hasNext()) {
        ...
        if (xtr.isStartElement() && "definitions".equalsIgnoreCase(xtr.getLocalName())) {
          ...
          if (xtr.isStartElement() && "startEvent".equalsIgnoreCase(xtr.getLocalName())) {
            ...
            } else if ("org.alfresco.repo.workflow.activiti.script.AlfrescoScriptDelegate".equalsIgnoreCase(xtr.getAttributeValue(
  ...
  private SubProcess parseSubProcess(XMLStreamReader xtr) {
    ...
    if (StringUtils.isNotEmpty(xtr.getAttributeValue(null, "triggeredByEvent")) && "true".equalsIgnoreCase(xtr.getAttributeValue(null, "triggeredByEvent"))) {
  ...
  private boolean isMailScript(String script) {
    boolean isMailScript = false;
    if (script != null) {
      if (script.contains("var mail = actions.create(\"mail\");") && script.contains("mail.execute(bpm_package);")) {
  ...
  private ServiceTask parseServiceTask(XMLStreamReader xtr) {
    ...
    final String extensionIdAttributeValue = xtr.getAttributeValue(ACTIVITI_EXTENSIONS_NAMESPACE, ExtensionConstants.ATTRIBUTE_NAME_SERVICE_TASK_EXTENSION_ID);
    if (extensionIdAttributeValue != null) {
      serviceTask.setExtensionId(extensionIdAttributeValue);
    }
  ...
  private static List<ActivitiListener> parseListeners(XMLStreamReader xtr, Process activeProcess) {
    ...
    try {
      ...
      while (readyWithListener == false && xtr.hasNext()) {
        ...
        if (xtr.isStartElement() && ("executionListener".equalsIgnoreCase(xtr.getLocalName()) || "taskListener".equalsIgnoreCase(xtr.getLocalName()))) {
  ...
  private BusinessRuleTask parseBusinessRuleTask(XMLStreamReader xtr) {
    ...
    if (xtr.getAttributeValue(ACTIVITI_EXTENSIONS_NAMESPACE, "exclude") != null) {
      businessRuleTask.setExclude(Boolean.valueOf(xtr.getAttributeValue(ACTIVITI_EXTENSIONS_NAMESPACE, "exclude")));
    }
    if (xtr.getAttributeValue(ACTIVITI_EXTENSIONS_NAMESPACE, "resultVariableName") != null) {
      businessRuleTask.setResultVariableName(xtr.getAttributeValue(ACTIVITI_EXTENSIONS_NAMESPACE, "resultVariableName"));
    }
